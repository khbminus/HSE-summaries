...
Можно упихать аж 9 домашек (скорее будет 5-7).
...
1. glvalue (generalized left). То у чего есть имя + адрес в памяти.
...
интуиция такая: `lvalue = rvalue`. `Xvalue` -- то, что получается после мува. Обладает свойствами `glvalue`: к нему
...
Выглядит вот так: `Foo(const Foo& other);`. Вызывается при создании из `lvalue`: `direct init`, `copy init`, ...

### Перемещающий конструктор

Выглядит вот так `Foo(const Foo&& other)`. Вызывается при создании из rvalue. Важный момент: внутри
конструкторов `other` -- `lvalue`
...
А вот стрелочка: унарный оператор, возвращаюсь что-то, у чего есть стрелочка (указатель). 