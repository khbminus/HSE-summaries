...
\end{itemize}
\begin{remark}
    $a^n = 1 \Rightarrow a \in R^*$. Т.к. тут записано, что  $a \cdot a^{n-1} = 1$ --- то есть он обратим.
\end{remark}
\begin{definition}
...
Эффективно вычислим $\varphi(n)$:
 ...
 \begin{proof}
     ...
     \begin{enumerate}
	     ...
	     С другой стороны, сама группа $\langle S \rangle$, которую мы описали в предыдущей теореме, является корректной подгруппой $G$, т.е. $H = \langle S \rangle \Rightarrow H \supset S \land H \le G$. Следовательно:  \[
 ...
 \slashn
    ...
    \end{statement}
    \begin{proof}
	    Пусть $(|G_1|, |G_2|) > 1$. Тогда $\forall a \in G_1, b \in G_2$ $a^{|G_1|} = e_{G_1} \land b^{|G_2|} = e_{G_2} \Rightarrow (a, b)^{\lcm(|G_1|, |G_2|)} = (e, e) \Rightarrow \forall x \in G_1 \times G_2: ord(x) \le \lcm(|G_1|, |G_2|) < |G_1| \cdot |G_2| = |G_1\times G_2| \Rightarrow G_1 \times G_2$ --- не циклическая.
\end{proof}
\begin{remark}
...
\end{remark}
Теперь докажем теорему о том, в каких случаях мультипликатиная группа вычетов циклическая.
\begin{proof}
    $n = p_1^{\alpha_1} \ldots p_k^{\alpha_k}$. Тогда $|(\Z / p_i^{\alpha_i} \Z)^*| = p_i^{\alpha_i} - p_i^{\alpha_i - 1} \divby 2$, кроме случая  $p_i = 2, \alpha_i = 1$. 
