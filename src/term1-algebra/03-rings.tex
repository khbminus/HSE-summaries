...
\end{statement}
\begin{definition}
    $R$ --- кольцо, тогда  $R^* = \{a \in R | \exists a^{-1}\}$ --- группа обратимых элементов.
\end{definition}
\begin{remark}
    $a^n = 1 \Rightarrow \in R^*$. Т.к. тут записано, что  $a \cdot a^{n-1} = 1$ --- то есть он обратим.
\end{remark}
\slashn
$R$ --- конечное кольцо.  $a \in R^* \Rightarrow a^{|R^*|} = 1$. Следствие из теоремы Лагранжа. Посчитаем  $|R^*|$. Это $\varphi(n)$ --- функция Эйлера.
\begin{remark}
