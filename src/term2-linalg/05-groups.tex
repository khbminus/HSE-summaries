...
\begin{enumerate}
    \item $|G| = ?$.
    \item membership test: если  $\pi \in S_n$, то верно ли, что  $\pi \in G$. Если да, то какое разложение по базису? То есть $\pi = \prod \pi_i^{\pm 1}$.
\end{enumerate}

Идея: 
\begin{enumerate}
    \item $G \acts \{1, 2, \ldots, n\}$. Тогда $|G| = |G \cdot 1| \cdot |G_1| = |G\cdot 1| \cdot |(G_1)\cdot 2| \cdot |G_{1,2}| \cdot \ldots = \prod |G_{1,2,\ldots,k} \cdot (k+2)|$

        $G\cdot 1$ легко знаем: просто посчитаем все графы  $\pi_i$. 

        Но встает вопрос, выписать системы образующих для $G_1, (G_1)_2, \ldots, (G_{1,2})_3$.
\end{enumerate}
\begin{lemma}[Лемма Шрайера]
    $G$ --- конечная группа,  $H \le G$, причем $G = \langle g_1, \ldots, g_т \rangle$

    $G / H = \{x_1H, x_2H,\ldots, x_kH\}$, $x_1, x_2,\ldots, x_k$ --- система образующих представителей классов, $x_1 = e$.

    Положим $\overline{g} = x_i$, если  $gH = x_iH$. Тогда  $\langle \overline{g_lx_i}^{-1} g_l x_i \rangle_{\substack{i = 1,\ldots, k\\ l=1..n}}$
\end{lemma}
\begin{proof}
    Пусть $a \coloneqq g_lx_o = a, b \coloneqq \overline{g_lx_i}$.  $b^{-1}a \in H$, так как  $bH=aH$.

    Пусть  $h \in H$. $h = g_{i 1} \cdot g_{i 2}, \ldots, g_{i s}$

    Запишем для каждого $x_{i(l-1)}^{-1}g_{il}x_{il}$. Заметим, что  $x_{i0}^{-1}h = x_{i 0}^{-1}g_{i 1} x_{i 1} \ldots x_{i l - 1}^{-1} g_{il} x_{i l}$.

    Тогда определим  $x_{il} = \overline{g_{i l+1} x_{i l+1}}$.

    я хочу к маме :(
\end{proof}

\begin{exerc}[Частный случай]
    $G \acts M$,  $m \in M$.  $G = \langle g_1, \ldots, g_n\rangle$.

    $G \cdot m = \{ m=m_1, m_2, \ldots \}$, $\forall i \exists x_i\!: x_im = m_i$.

     $H = G_m$, тогда  $G / H = \{x_1H, x_2H, \ldots\}$. По лемме Шрайера можно найти систему образующих.

     Построим сильную базу для $G$:  $G \cdot 1 = \{i_{11}, i_{12}, \ldots, i_{1k}\}$, $x_{1l} \cdot 1 = i_{1l}$.

     Нашли по Шрайеру образ для  $G_1$. $G_1 \cdot 2 = \{ i_{21}, i_{22}, \ldots, i_{2k_2}\} x_{2l} \cdot = i_{2l}$. и так далее. $\{x_{ij}\}_{\substack{i=1..n-1}}$ --- сильная база для  $G$.
    
     Теперь ответим на два главных вопроса:
     \begin{enumerate}
         \item $|G| = |G_1| \cdot |G_1\cdot 2| |G_{12} \cdot 3| \cdot \ldots = k_1 \cdot k_2 \cdot k_{n-1}$.

             membership test: $g \in S_n\!: g(1) = a_1, x_{1a_1}(1)g \in G$. $x_{1a(1)}^{-1} g \in (S_n)_1$. $x_{1a(1)}g(2) = a_2 = x_{2a_2}(2)$. И так далее\dots

             В конце, если $x_{n-1 a_{n-1}}^{-1} x_{n-2 a_{n-2}}^{-1} \ldots x_{a a_1}^{-1} g= id$.
...
\begin{exerc}
    ...
    $|G| = 2^m$,  $\forall g \in G \{l\} \ord g = 2, g_1, \ldots, g_k \in G$. $|\langle g_1, \ldots, g_k \rangle| = 2^k \implies \forall$ с.о. образующих имеет $\ge m \frac{n}{2}$ элементов.
\end{exerc}
\begin{theorem}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $\Gamma'$ --- ацикличная = ок.  $g_{k+1, i} = g_{k, i}, g_{k+1, i+1} = h_{k+1}$.
...
\end{definition}
\begin{definition}
    $a, b \in G$ коммутатор  $s$ и  $b$ ---  $[a, b] = a^{-1}b^{-1}ab = (ba)^{-1}ab$.
\end{definition}
\begin{definition}
...
\begin{theorem}[Свойства центра и коммутанта]
    \begin{enumerate}
        ...
        \item[$1_2$.] $[G_1, G_2] \trianglelefteq G$.
        \item[$2_1$.] $Z(G)$ --- абелева группа.
        \item[$2_2$.] $G / [G, G]$ --- абелева.
        \item[3.]  $H \trianglelefteq G$,  $G / H$ --- абелева  $\implies H \ge [H, H]$ (универсальное свойство коммутанта).
    \end{enumerate}
\end{theorem}
\begin{proof}
    \begin{enumerate}
        \item[$1_1$.] $e \in Z(G)$.  $a, b \in Z(G), \forall c \in G ac = ca, bc = cb \implies (ab)c = a(bc) = a(cb) = (ac)b = (ca)b \implies ab \in Z(G)$.

            $a \in Z(G) \implies a^{-1} \in Z(G)$. Нормальность проверяется легко: $g^{-1}hg = hg^{-1}g = h$.
        \item[$1_2$.] $[G, G] \le G$ --- по определению. 

            Теперь заметим, что $x = [a, b] = a^{-1}b^{-1}ab$. Рассмотрим  $g^{-1}xg = g^{-1}a^{-1}b^{-1}abg = \ldots = [a', b']$, где $a' = g^{-1}ag, b' = g^{-1}bg$.

            $[a, b]^{-1} = (a^{-1}b^{-1}ab)^{-1} = b^{-1}a^{-1}ba = [b, a] \implies \forall $ элемент  $[G, G]$ --- произведение коммутаторов. 

             $x \in [G, G]$, тогда  $x = [a_1, b_1][a_2,b_2]\ldots[a_n, b_n]$, $g^{-1}xg = g^{-1}[a_1, b_1][a_2, b_2]\ldots[a_n, b_n]g = \ldots = [a_1', b_1']\ldots \in [G, G]$.
         \item[2.] $G / [G, G] = \overline{a}, \overline{b}$.  $\overline{a} \cdot \overline{b} = \overline{a}\overline{b}\overline{b^{-1}a^{-1}}\overline{ba} = \overline{a}\overline{b}\overline{b^{-1}}\overline{a^{-1}}\overline{ba} = \overline{ba}$.
         \item[3.] $G / H$ --- абелева  $\forall a, b \in G\quad \overline{a_H} \overline{b_H} = \overline{b_H}\overline{a_H}$.  $\overline{a_H}^{-1}\overline{b_H}^{-1} \overline{a_H}\overline{b_H} = \overline{e} \implies \overline{a^{-1} b^{-1}ab} = \overline{e} \implies a^{-1}b^{-1}ab \in H$
         \item[4.] Тут нужна картинка, но я краб.
    \end{enumerate}
\end{proof}
\begin{theorem}
    $A$ --- коммутативна абелева  $\implies$  $A \cong \Z / p_1^{\alpha_1} \times \ldots$.
\end{theorem}
\begin{proof}
    Нет.
\end{proof}
\begin{example}
    $S_n$,  $n \ge 5$.
    $[S_n, S_n] = A_n$,  $S_n / A_n = C_2$,  $[A_n, A_n] = A_n$ --- соверешенная.

     $Z(S_n) = \{e\} = Z(A_n) = \{e\}$.

     $n = 4$.  $[S_4, S_4] = A_4$.  $[A_4, A_4] = D_4 = \langle (12), (34), (13), (34) \rangle$. $[D_4, D_4] = \{e\}$.
\end{example}
\begin{example}
    $G$ --- конечная  $\to G_1 =G / Z(G) \to G_2 = G_1 / Z(G_1) \to \ldots$.
...
\begin{statement}
    ...
    Если существует $k$, такое  $G_k = \{e\}$(первый или второй случай), то  $G$ называется нильподентной группой. 
    Если существует $k$, такое  $G_k = \{e\}$(третий случай), то  $G$ называется разрешимая группой. 
\end{statement}
\begin{example}
    $S_4$ --- разрешима, но не нильподентна. 
\end{example}
\begin{example}
    Рассмотрим нижнетреугольные матрицы $\le GL_n(K)$  --- нмльподентная подгруппа.

    $Z(GL_n(K)) = \{k\cdot E\}$ --- скалярные матрицы.  $GL_n / Z(GL_n) = PGL_n$,  $Z(PGL_n) = \{e\}$.

     $[GL_n, GL_n] = SL_n(K), [SL_n, SL_n] = SL_n(n)$.
\end{example}
\begin{theorem}
    $G$ --- конечна. Тогда

    Следующие условия равносильный: 
    \begin{enumerate}
        \item $G$ --- разрешима.
...
\end{proof}
\begin{theorem}
    $G$ --- конечная, тогда  $|G| = p^n \implies G$ --- нильподентна. 
\end{theorem}
\begin{proof}
...
\end{lemma}
\begin{proof}
    $G \acts G$ сопряжениями. $g \neq m = g \cdot m g^{-1}$.   $O_i$ --- орбита,  $O_1 = G \cdot e = \{e\}$.  $|O_1| + |O_2| + \ldots = p^n \implies \exists i \neq 1\!: |O_i| = 1$, так как любая орбита имеет порядок либо $1$, либо степень $p$. 

    Теперь пусть $|G| = p_1^{\alpha_1} \ldots p_k^{\alpha_k}$. По первой теореме Силова $\exists |G_1| = p_1^{\alpha_1}, \ldots$.
\end{proof}
\begin{theorem}
...
\begin{proof}
    ...
    Иначе $\forall g \neq e \ord g = p$.  Тогда $Z(G) = G$ (тогда $G$ --- абелева) или $|Z(G)| = p$. Тогда  $Z(G) = \langle a \rangle, \ord a = p$,  $b \notin Z(G), \ord b = p$.  $\langle a, b\rangle p^2 \implies \langle a, b \rangle = G \implies G$ --- абелева.
\end{proof}
\begin{remark}
    $G \cong \Z / p^2 \Z$ или $\Z / p \Z \times \Z / p \Z$.
\end{remark}
\begin{example}
    $|G| = p^3$.
...
\begin{definition}
    ...
    $g = g_1g_2$, $(g_1g_2) \cdot (g_1' g_2') = g_1g_1'(g_1')^{-1}g_2g_1'g_2' = (g_1g_1')((g_1')^{-1}g_2g_1')g_2' = \underbrace{(g_1g_1')}_{\in G_1}\underbrace{(f_{g_1'}(g_2)g_2')}_{\in G_2}$.

    Полупрямые произведения задаётся $G_1, G_2$ гомоморфизмами. $G_1 \to Aut(G_2)$.
\end{definition}
