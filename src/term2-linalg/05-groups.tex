...
\begin{proof}
    ...
    $\pi_{hf} = \pi_h \cdot \pi_f\quad$.  $h \cdot g_{\pi_f}(i) = h(fg_i) = (hf)g_i = g_{\pi_{h,f}(i)}$.
\end{proof}
\begin{remark}
...
\begin{proof}
    ...
    Построим  $\widetilde{f}\!: G_1 / \ker f  \to \Im f$. $\widetilde{f}(b \cdot  \ker f) = f(b)$.
    \begin{enumerate}
...
Верно ли, что в  $G$ есть подгруппа порядка  $d$? Нет, например,  $A_4$, подгруппа  порядка $6$. Но если $d = p^k$, то ок.
 
\begin{theorem}[Теорема Коши]
    $|G| \divby p, p$ --- простое,  $\implies \exists g \in G\!: \ord g = p$.
\end{theorem}
\begin{proof}
    $M = \{(g_1, \ldots, g_p) \mid g_i \in G, g_1g_2\ldots g_p = e \}$. $|M| = |G|^{p-1} \divby p$.  $C_p \acts M\!: t(g_1, g_2, \ldots, g_p) = (g_2, \ldots, g_p, g_1)$. 

   $|C_p \cdot x| = \frac{|C_p|}{|(C_p)_x|} = \left[ \begin{array}{l} 1 \\ p \end{array} \right.$. $|M| = \sum \text{длина орбиты} = \underbrace{1+1+\ldots+1}_{x} + p + \ldots + p \divby p$. 

   Орбиты длины 1 --- $\{(x, x, \ldots, x) \mid x^p = e\}$. Причем, заметим, что единица подходит $\implies$ элемент порядка  $p$ существует.
\end{proof}
