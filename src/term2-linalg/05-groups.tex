...
\begin{proof}
    ...
    Запишем для каждого $x_{i_{(l-1)}}^{-1}g_{i_l}x_{i_l}$. Заметим, что  ${\color{red}x_{i_0}^{-1}h} = x_{i_0}^{-1}g_{i_1} x_{i_1} \ldots x_{i_{(l - 1)}}^{-1} g_{il} x_{i_l}$.

    Тогда определим  $x_{i_l} = \overline{g_{i_{l+1}} x_{i_{l+1}}} \Rightarrow$ {\color{blue}$x_{i_{(l-1)}}^{-1}g_{i_{l}} x_{i_{l}}$}~--- элемент нашей системы образующих.
    $\Rightarrow x_{i_0}^{-1}h = $ произведение наших образующих $\in H$.
    $h \in H \Rightarrow {\color{red}x_{i_0} \in H} \Rightarrow x_{i_0} = e$.
